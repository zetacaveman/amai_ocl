"""Unit tests for coordinator role assignment."""
...
from aimai_ocl.schemas import ActionRole, AuditEventType
...
class CoordinatorTests(unittest.TestCase):
    ...
    def test_build_audit_event_contains_role_assignments(self) -> None:
        ...
        - metadata includes decision role and mode
        """
        plan = self.coordinator.plan_turn(
            round_id=0,
        ...
        self.assertEqual("default", event.metadata["mode"])

    def test_seller_only_coordinator_forces_seller_decision(self) -> None:
        """Input: buyer question and deadline round.

        Expected output:
        - decision role still stays seller in seller-only ablation mode
        """
